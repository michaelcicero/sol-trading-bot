...
class RSITrader:
                ...
                if ix['programId'] in [
                    ...
                    if len(data) >= 17 and data[0] in [9, 1]:
                        ...
                        try:
                            ...
                            popcat_index = next(i for i, key in enumerate(account_keys) if key == TARGET_MINT)
    ...
    async def execute_trade(self, input_mint: str, output_mint: str, amount: float):
        ...
        for attempt in range(3):
            try:
                ...
                # Enhanced error handling for custom program errors
                err_str = str(e)
                if "custom program error: 0x1" in err_str or "custom program error: 0x28" in err_str:
                    logging.error(f"Trade failed due to Solana custom error (likely insufficient funds or token account issue): {err_str}")
                    break  # Don't retry further for these errors
                logging.error(f"Trade error: {err_str[:200]}")
    ...
    async def _get_swap_data(self, quote):
        async with self.session.post(
