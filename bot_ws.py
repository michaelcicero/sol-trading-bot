...
TOKEN_DECIMALS = 9
TOKEN_SYMBOL = "POPCAT"
MIN_PRICE_CHANGE = 0.001  # 0.1% minimum price movement
MIN_ATR_VALUE = 0.00001    # 0.001% minimum volatility
...
JUPITER_PRICE_API = "https://lite-api.jup.ag/price/v2?ids=7GCihgDB8fe6KNjn2MYtkzZcRjQy3t9GHdC8uHYmW2hr&vsToken=So11111111111111111111111111111111111111112"
JUPITER_QUOTE_API = "https://lite-api.jup.ag/quote/v1"
...
class TokenTrader:
    ...
    async def fetch_initial_price(self):
        try:
            async with self.session.get(JUPITER_PRICE_API) as resp:
                data = await resp.json()
                if 'data' in data and TARGET_MINT in data['data']:
                    price = float(data['data'][TARGET_MINT]['price'])
                    for _ in range(10):
                        self._update_price_data(price, source='Jupiter')
        except Exception as e:
            logging.error(f"Price initialization failed: {str(e)}")

    def _update_price_data(self, price, source=None):
        async with self.price_lock:
            ...
            logging.debug(f"Price updated: {price:.6f}. History now {len(self.price_history)} entries")
    ...
    async def _process_websocket_message(self, message):
        try:
            ...
            if price and 1e-8 < price < MAX_REASONABLE_PRICE:
                self._update_price_data(price, source='WS')
        ...
        async with self.atr_lock:
            ...
            try:
                ...
                if not self.short_atr_values:
                    raise ValueError("Short ATR values not populated")
