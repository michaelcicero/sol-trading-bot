...
class TokenTrader:
    def __init__(self):
        ...
        # Reduce data collection requirements for faster startup
        self.price_history = deque(maxlen=100)
        self.short_atr_period = 14  # Back to original
        self.long_atr_period = 28  # Back to original
        ...
        self.base_trade_size = 0.1
        self.cooldown = 180
        self.min_sell_amount = 50
        self.sell_percentage = 0.85
        self.sell_threshold = 1.035
        self.trailing_stop_pct = 0.96
    ...
    async def get_balance(self, mint: str):
        try:
            if mint == SOL_MINT:
                ...
                # Try direct balance method first
                try:
                    payload = {
                        "jsonrpc": "2.0",
                        "id": 1,
                    ...
                    async with self.session.post(
                        ...
                        if resp.status == 200:
                            ...
                            if "result" in result and "value" in result["result"]:
                                raw_balance = int(result["result"]["value"])
                                token_balance = raw_balance / (10 ** TOKEN_DECIMALS)
                                # Apply POPCAT-specific correction
                                if mint == TARGET_MINT:
                ...
                # Try getTokenAccountsByOwner with jsonParsed
                try:
                    payload = {
                        "jsonrpc": "2.0",
                        "id": 1,
                    ...
                    async with self.session.post(
                        ...
                        if resp.status == 200:
                            ...
                            if "result" in result and "value" in result["result"]:
                                ...
                                for account in result["result"]["value"]:
                                    try:
                                        ...
                                        if "info" in parsed_data and "tokenAmount" in parsed_data["info"]:
                                            token_amount = float(parsed_data["info"]["tokenAmount"]["uiAmount"])
                                ...
                                return round(total_balance, 1)
                except Exception as e:
                    logging.error(f"JSON parsed token accounts failed: {str(e)}")
                
                # Fallback to manual parsing (base64)
                ...
                total_balance = 0.0
                logging.info(f"🔍 Checking {len(resp.value)} token accounts for {TOKEN_SYMBOL}")
                
                for account in resp.value:
                    try:
                        # Handle different data formats
                        if isinstance(account.account.data, list) and len(account.account.data) > 0:
                            # [data_str, encoding] format
                            data_str = account.account.data[0]
                        else:
                            # Direct string format
                            data_str = account.account.data
                        
                        account_data = base64.b64decode(data_str)
                        # Token accounts have: mint(32 bytes), owner(32 bytes), amount(8 bytes)
                        # Extract the amount bytes (offset 64, length 8)
                        if len(account_data) >= 72:  # Ensure enough bytes
                ...
                return round(total_balance, 4)  # Show 4 decimal places for precision
        except Exception as e:
    ...
    async def calculate_atr(self, period, atr_values):
        if len(self.price_history) < period * 2:
            # Base minimum on percentage of current price
            current_price = self.price_history[-1]['close'] if self.price_history else 0.003
            return max(MIN_ATR_VALUE, current_price * 0.001)  # At least 0.1% of price
        ...
        if not atr_values:
            ...
            # Make ATR relative to current price (NATR approach)
            current_price = self.price_history[-1]['close']
            scaled_atr = max(initial_atr, current_price * 0.001)  # At least 0.1% of price
            return scaled_atr
        ...
        # Scale to current price - ensure minimum volatility threshold
        current_price = curr['close']
        scaled_atr = max(new_atr, current_price * 0.001)  # At least 0.1% of price
        ...
        while True:
            try:
                ...
                if len(self.price_history) < required_history:
                    ...
                    await asyncio.sleep(5)  # Faster data collection
                    continue
