...
class TokenTrader:
    def __init__(self):
        ...
        # Price history stores OHLC values
    ...
    async def calculate_atr(self):
        ...
        if not self.atr_values:
            ...
            for i in range(self.atr_period):
                ...
                tr_values.append(tr)
            initial_atr = sum(tr_values) / self.atr_period
    ...
    async def get_token_price(self):
        try:
            ...
            async with self.session.get(f"{self.base_url}/quote", params=params) as resp:
                ...
                # Track OHLC (simplified single-price candles)
    ...
    async def trading_strategy(self):
        while True:
            try:
                ...
                logging.info(f"{TOKEN_SYMBOL}: {price:.2f} | Avg: {avg_price:.2f} | ATR: {atr:.2f} | Thresh: {dynamic_threshold:.2f}")

                # Buy logic with dynamic threshold
                ...
                # Sell logic with minimum profit check
                if token_balance > self.min_sell_amount and (
                    price > avg_price * self.sell_threshold or price < self.trailing_peak * self.trailing_stop_pct
                ):
                    ...
                    if trade_value_sol >= self.min_profit_sol:
                        logging.info(f"💰 Selling {sell_amount:.0f} {TOKEN_SYMBOL} (Value: {trade_value_sol:.4f} SOL)...")
                        if await self.execute_trade(TARGET_MINT, SOL_MINT, sell_amount):
                            self.last_trade = current_time
                            self.trailing_peak = 0.0  # Reset only after full sell
                    else:
