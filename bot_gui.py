...
from prometheus_client import start_http_server, Gauge  # Added for metrics
...
class TokenTrader:
    def __init__(self):
        ...
        self.max_trade_duration = timedelta(minutes=90)
    ...
    async def calculate_position_size(self, sol_balance, atr, current_price):
        ...
        atr_adjusted = max(atr, current_price * 0.005)
        return min(risk_amount / atr_adjusted, sol_balance * 0.25)
    ...
    async def monitor_balances(self):
        ...
        while True:
            try:
                ...
                self.sol_balance_gauge.set(current_sol_balance)
                self.popcat_balance_gauge.set(current_token_balance)
                if current_price:
                    self.popcat_price_gauge.set(current_price)
